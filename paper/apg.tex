...
edge offset is two. That does \emph{not} by itself bound the span: the closed
index sequence $0,2,4,3,1,0$ has five steps of size at most two and touches five
levels. The bound we use is \emph{measured} instead --- over every spliceable
order at $d = 1,2,3,5$ the largest span is two, so a facial walk touches at most
three consecutive copies --- and it is gated as a measurement. The fresh copies lie in $(\mathrm{cut}, \mathrm{cut}+d]$, and
