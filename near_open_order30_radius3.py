#!/usr/bin/env python3
"""Linux-only final exact radius-3 expansion of the order-30 frontier."""
...
def load_order30_radius2_frontier(
    seed_path: Path,
    seed: dict[str, object],
    k4_log: dict[str, object],
    radius2_log: dict[str, object],
) -> tuple[map_search.FixedMap, list[dict[str, object]]]:
    """Replay every recorded order-30 radius-2 parent hash and score."""

    fixed, k4_parents = near_open_order30_radius2.load_order30_k4_frontier(
        seed_path, seed, k4_log
    )
    result = radius2_log.get("result")
    if not isinstance(result, dict) or not result.get("complete"):
        raise ValueError("order-30 radius-2 result is absent or incomplete")
    if result.get("parent_state_hashes") != [
        parent["state_sha256"] for parent in k4_parents
    ]:
        raise ValueError("order-30 radius-2 parent hashes do not reproduce")
    states = result.get("frontier_states")
    if not isinstance(states, list) or len(states) != FRONTIER_SIZE:
        raise ValueError("order-30 radius-2 frontier must contain 64 states")
    keys: list[tuple[int, str]] = []
    for state in states:
        alpha = state.get("alpha")
        if not isinstance(alpha, list) or len(alpha) != len(fixed.dart_vertex):
            raise ValueError("order-30 radius-2 frontier alpha is malformed")
        digest = near_opening._state_sha256(alpha)
        if digest != state.get("state_sha256"):
            raise ValueError("order-30 radius-2 frontier hash does not reproduce")
        breakdown = map_search.score_breakdown(fixed, alpha)
        if breakdown != state.get("breakdown"):
            raise ValueError("order-30 radius-2 frontier score does not reproduce")
        if not map_search._abstract_graph_ok(fixed, alpha):
            raise ValueError("order-30 radius-2 frontier is not graph-valid")
        keys.append((breakdown["total"], digest))
    if keys != sorted(keys) or len(set(keys)) != len(keys):
        raise ValueError("order-30 radius-2 frontier is not ordered and distinct")
    manifest = parent_manifest_sha256(states)
    if manifest != EXPECTED_PARENT_MANIFEST_SHA256:
        raise ValueError(f"order-30 radius-2 manifest changed: {manifest}")
    return fixed, states


def main() -> int:
    if platform.system() != "Linux":
        raise SystemExit("near_open_order30_radius3.py is Linux-only")
    ...
    fixed, parents = load_order30_radius2_frontier(
    ...
    self_check = {
        ...
        "seed_file_hash_matches": near_open_order30_radius2.file_sha256(args.seed)
    ...
    required = {
        ...
        "edges_per_parent": near_open_order30_radius2.EXPECTED_EDGES_PER_PARENT,
        "edge_pairs_per_parent": near_open_order30_radius2.EXPECTED_EDGE_PAIRS_PER_PARENT,
    ...
    if self_check != required:
        raise AssertionError(f"order-30 radius-3 self-check failed: {self_check}")
    for index, block in enumerate(successes):
        bt.write_json(args.output_directory / f"block_{index:04d}.json", block)
    replay = (
        f"python3 near_open_order30_radius3.py --seed {args.seed} "
    ...
    bt.write_json(
        args.log,
        {
            "claim_scope": (
                ...
                "states in the recorded order-30 radius-2 frontier. A miss is "
                "bounded to this seed family and is not nonexistence."
            ),
            "decision": {
                "order30_seed_family_closed": not successes,
                "next_radius_started": False,
                "radius4_forbidden_by_operator": True,
                "reason": (
                    "Witness found; stop for verification/reporting."
                    if successes
                    else "No witness at final bounded radius 3; close this order-30 seed family regardless of numeric improvement."
                ),
            },
            "source": radius2_log["source"],
            "seed_file_sha256": near_open_order30_radius2.file_sha256(args.seed),
    ...
    print(
        f"PASS order30 radius3 complete={stats['complete']} "
