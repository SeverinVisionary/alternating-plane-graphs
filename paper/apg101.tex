...
\end{tikzpicture}
\caption{Two quadrilaterals sharing the white cut vertex. The numbers are face
sizes; the unbounded face is marked three times, once in each of the two corners
it occupies at the white vertex and once further out. Its boundary walk
therefore meets that vertex twice, and has length $8$, the number of edge-side
...
edge leaves $v+f-e = 2$. Note that $2$ is the total \emph{signed} excess. The
identity does not in general identify it with the sum of the positive excesses,
and on its own it gives no upper bound on an individual one.
\end{proof}
...
belong to the same face. In this local configuration $a_{1}$, drawn heavy, is the
only bridge, so $F_{5}$ and $F_{1}$ are the same face and the colour is unchanged across it,
...
$F^-,F^+$. In each row the side carrying only two values is bounded by
$\tfrac7{12}$ at once. On the other side, alternation makes the two values met
at an edge distinct, and bipartiteness --- respectively the parity lemma, using
that there are no bridges --- makes them even; being distinct and even they are
at least $4$ and $6$, so that side is bounded by $\tfrac5{12}$. The roles of vertices and faces are exchanged between
...
where real work is done, is to bound the number of edges of possibly positive
excess assigned to each bridge.
...
\begin{tikzpicture}[scale=1.05,>=latex]
  ...
  \draw[->,thick] (0,0) arc (0:340:0.95);
  \draw[->,thick] (2.5,0) arc (180:-160:0.95);
  \node at (-0.95,1.32) {\footnotesize $W_A$};
  \node at (3.45,1.32) {\footnotesize $W_C$};
  \node at (1.25,-1.30) {\footnotesize $F$};
\end{tikzpicture}
\caption{The arrowed loops are the exposed facial walks $W_A$ and $W_C$ of the
two components $A$ and $C$ of $G-a$, whose internal edges are omitted; the loops
are schematic and these walks need not be cycles. The face $F$ carrying the bridge $a=uw$ follows $W_A$,
...
In units of $\tfrac1{24}$ the bounds of Table~\ref{tab:cases} read $+1$ for a
positive-eligible edge and $-4$ for a bridge, so even a bridge carrying its full two
...
\end{tikzpicture}
\caption{The assignment at the degree-$3$ end $u$ of a positive-eligible edge.
Parity leaves $1$ or $3$ bridges at $u$, and the positive-eligible edge at $u$
is itself a non-bridge, so there is exactly one bridge $b=uw$; the remaining two
edges $a_1,a_2$ at $u$ are then non-bridges with an end of degree $3$, hence
positive-eligible, and both are assigned to $b$ (dashed arrows, which are
assignments and not edges of the graph); this is the worst case for the
capacity bound. Alternation forbids
$\deg w = 3$, so $b$ receives nothing through $w$; the further edges at $w$ are
dotted. This is a local configuration in a hypothetical counterexample, not a
graph in its own right.}\label{fig:assign}
\end{figure}
...
degree-$5$ vertex $z_i$ is the exterior face, and join both $z_1$ and $z_3$ to
the \emph{single} vertex $z_2$ by two bridges drawn in the exterior. That both
bridges land on the same $z_2$ is what gives their ends different degrees:
$\deg z_2 = 7$ and
