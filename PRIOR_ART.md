...
Re-run before preparing a journal submission. Narrower than the audits above:
this one asks only whether anyone has published a settlement of Conjecture 10.1,
10.2 or 10.3 since the last check. **Nothing found.**

| source | query | result |
| --- | --- | --- |
| OpenAlex | works citing `W1798145357` | **1** |
| Semantic Scholar | citations of `10.26493/1855-3974.584.09a` | **2** |
| arXiv API | `all:"alternating plane graphs"` | **0 preprints** |
| the authors' own page | [althofer.de][ath] | last updated **December 2014** |

The two citing works, opened rather than counted:

* **"Review and Assessment of Digital Twin-Oriented Social Network
  Simulators"**, *IEEE Access* (2023), [doi:10.1109/access.2023.3312129][c1] --
  unrelated, as recorded in the 2026-08-30 audit.
* **"Computer-assisted graph theory: a survey"** (2025),
  [arXiv:2508.20825][c2]. Cites the paper once, as a single line in a table of
  computer-assisted searches -- "Alternating plane graphs [3]" -- with no
  discussion of the conjectures. Checked in the full PDF this time, not the
  abstract.

### Two apparent leads that dissolved

Both are recorded because each looked, for a moment, like a priority problem.

* A web search returned **arXiv 2409.19398** as a 2024 revision of the source
  paper by the same five authors. **It is not**: that identifier is an
  information-theory paper on arbitrarily varying channels. Confirmed by
  fetching it.
* [althofer.de][ath] says a *"revised version ... exists and is under
  revision"*. The page's own last update is 1 December 2014, so the sentence
  describes the pre-publication state of the 2015 paper itself.

### Standing caveat, unchanged

Indexes lag and do not reach every venue. What is claimed is that three
independent indexes, the arXiv and the authors' own page show nothing as of
2026-09-06. **The original authors have still not been contacted**, and they
remain the only source that would settle this definitively.

[ath]: https://althofer.de/alternating-plane-graphs.html
[c1]: https://doi.org/10.1109/access.2023.3312129
[c2]: https://arxiv.org/abs/2508.20825
