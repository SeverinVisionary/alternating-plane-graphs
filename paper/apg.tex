...
small-$k$ weak graphs. The case $k = 11$ is left open by the chain, exactly as in
\cite{AHSSV2015}, which needs its Lemma~9.4 for it.

This is emphatically \emph{not} a certification of the chain. A spuriously
\emph{stronger} face bound such as $5/12 - 1/1000$ reproduces the same
threshold exactly, so the calibration cannot detect an invalid strengthening ---
which is the dangerous direction. It rules out one specific failure mode and no
others.
