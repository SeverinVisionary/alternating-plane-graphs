...
Condition (d) of Definition~\ref{def:apg} is ambiguous at a bridge, since then
one face lies on both sides of an edge. Two readings are available.
...
Definition~\ref{def:apg}; under the \emph{weak reading}, (d) constrains only
pairs of distinct faces and bridges are permitted.
\end{convention}

This is not a quibble: the proof of Conjecture~10.1 in \S\ref{sec:101} uses
(C2), and we therefore prove in \S\ref{sec:c2} that a putative counterexample
must be bridgeless under \emph{either} reading. The result is thus independent
of the ambiguity.
...
\begin{proof}
Orders below $46$ are the published witnesses of \cite{AHSSV2015} together with
...
Definition~\ref{def:apg}. There are exactly five on $19$ vertices, and all five
are $3$-connected. A single one would have sufficed.
...
\texttt{plantri} or an equivalent generator restricted to the degree and
...
for checking by \texttt{plantri} or House of Graphs.
