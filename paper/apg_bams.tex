...
vertex. That is a statement about a single face, not about the graph: a
connected plane graph is $2$-connected if and only if \emph{every} facial
boundary walk is a cycle, so failing $2$-connectivity forces \emph{some} walk to
repeat a vertex but not every one --- two triangles sharing a cut vertex have
two faces that repeat nothing and an outer face that does. In
...
Figures~8--9). It does both, which shows the contradiction turns on
$d_1 \ge 3$ rather than on an over-count that would also have killed the
small-$k$ weak graphs. It is \emph{not} a certification: a spuriously
\emph{stronger} bound such as $5/12 - 1/1000$ reproduces the same threshold
exactly, so the check cannot detect an invalid strengthening, which is the
dangerous direction. The case $k = 11$ is left open by the chain, exactly as in
...
per-edge count; an earlier draft claimed it followed from a leaf-block argument,
which was unsound and is withdrawn.
...
When no vertex lies on a bridge the lemma reduces to the usual statement. In an $X,2$-alternating plane graph the face
...
has at least three vertices. Consider the facial boundary walk of $A$ incident
with the corner formerly occupied by $a$. It cannot have length $1$, which needs
a loop. Nor length $2$: that is either a digon of parallel edges, excluded by
simplicity, or a single edge traversed twice, and for two darts alone to form a
facial orbit both its endpoints have degree $1$, making the component $K_2$ ---
whereas $\deg_A u \ge 2$. So the walk has length at least $3$, and the same
holds on the other side. The face $F$ carrying $a$ is traced as that walk of
$A$, then $a$, then the corresponding walk of the other component, then $a$
again, so by Convention~\ref{conv:c1}
\[
...
resolves to the current version; this paper describes version~$1.0.3$,
\href{https://doi.org/10.5281/zenodo.22653691}{10.5281/zenodo.22653691}.
