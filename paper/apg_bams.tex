...
\begin{document}
\runningtitle{Three conjectures on alternating plane graphs}
...
We settle two and advance a third.

Conjecture~10.1, that no alternating plane graph has exactly two distinct vertex
degrees or exactly two distinct face sizes, is proved by one per-edge inequality
applied twice. With face size counted by boundary-walk length throughout, the
same conclusion is then proved again when face alternation is required only
between distinct adjacent faces, so that bridges are admitted.

Conjecture~10.2, that $(3,4,5)$-alternating plane graphs exist for every
$n \ge 20$, is settled by verified certificates at the $26$ open orders together
with a periodic capping lemma reaching order $48$ and every $n \ge 50$; these
close every $n \ge 46$ independently of the original paper.

For Conjecture~10.3 we certify $54$ orders by explicit witness and leave the
tail above $56$ open. We also exhibit a $(3,4,5)$-graph on $46$ vertices with a
separating pair. The fourth problem is untouched.
\end{abstract}
\classification{primary 05C10; secondary 05C30, 68R10}
\keywords{alternating plane graph, plane graph, rotation system, certificate verification}
\maketitle
...
Two are settled here, and the third is advanced but not closed.

\subsection*{Results}

\begin{itemize}
  \item Conjecture~10.1 is proved (Theorem~\ref{thm:101}), and again for the
        bridge-relaxed definition of \S\ref{sec:c2}.
  ...
  \item Conjecture~10.3 is \emph{not} settled here. Theorem~\ref{thm:103}
        gives $3$-connected witnesses at $54$ orders; the tail above $56$ is
        left open.
...
Large language models were used throughout this work and the extent is
material. By task:

\begin{itemize}
\item \emph{Software.} The four decision procedures, the search programs that
  found the certificates, and the supporting tooling were written with AI
  assistance.
\item \emph{Mathematics.} The step in \S\ref{sec:c2} that drops the (C2)
  hypothesis, extending Theorem~\ref{thm:101} to bridge-relaxed graphs, was
  proposed by an AI reviewer. The author re-derived it independently, and every bound it
  uses is confirmed by exhaustive search over the relevant degree and face-size
  ranges in exact rational arithmetic.
\item \emph{Adversarial review.} AI review found errors the author had missed,
  including three claims stated in earlier drafts that were false and were
  withdrawn. These are recorded at the files that made them rather than deleted.
\item \emph{Drafting.} The manuscript was drafted with AI assistance. The author
  has verified its mathematical content and is answerable for it and for this
  disclosure; the prose is not claimed to be the author's own sentence by
  sentence.
\end{itemize}

% ^^ AUTHOR: this replaced a stronger sentence -- "has read and revised every
%    sentence of it and is answerable for each one" -- which was not yet true
%    and so could not be published.  If you do read the manuscript end to end,
%    restoring the stronger wording is a one-line edit and is worth doing.

No AI system is an author of this paper, and responsibility for every claim in
it rests with the named author. The artifact~\cite{artifact} names the systems
and versions used, and states which of its own conclusions were taken on trust.

\subsection*{On the use of computation}

Two results assert the existence of specific graphs, and those graphs are large:
the certificates run to $110$ vertices. We state them as rotation systems, which
determine a plane map up to homeomorphism, and derive every claimed property
rather than storing it; separately written decision procedures recompute it on
every run --- three for the $(3,4,5)$ class and a fourth for the general class of
Definition~\ref{def:apg}, which Conjecture~10.3 needs at order~$19$. The proofs
in \S\ref{sec:101} and \S\ref{sec:pumping} are ordinary mathematics and can be
read without a computer; computation enters only in verifying the certificates
and in discharging the three finite hypotheses of Remark~\ref{rem:hypotheses}.
...
A plane map is encoded by its \emph{rotation system}~\cite{MoharThomassen}: for each vertex, its
...
what makes it correct at a cut vertex, where one face may occupy several
corners. Two such corners are never separated by a single non-bridge edge,
which is all that is needed here; across a bridge, consecutive corners do belong
to the same face, and there are no bridges under (C2).
...
have killed the small-$k$ graphs. It is emphatically \emph{not} a
certification: a spuriously \emph{stronger} face bound such as $5/12 - 1/1000$
reproduces the same threshold exactly, so the calibration cannot detect an
invalid strengthening --- the dangerous direction.
...
The span bound is taken on $\mathrm{TARGET}_n$, not on $S$; a bound measured on
$S$ would be circular, since it is the faces of $S$ that are in question. In
$\mathrm{TARGET}_n$ every face has at most five darts and the cover's largest
edge offset is two, both finite checks (Remark~\ref{rem:hypotheses}). A closed
sequence of at most five steps of size at most two has span at most four ---
reaching level $5$ needs three ascents, leaving two to descend five --- so
\emph{every face of $\mathrm{TARGET}_n$ lies inside five consecutive copies}.

Four is not sharp --- the largest span actually attained is two --- but
sharpness is not needed, because five copies is exactly what the hypothesis
supplies. The
fresh copies lie in $(\mathrm{cut}, \mathrm{cut}+d]$ and are translates of
$\mathrm{cut}$; since $\mathrm{cut}$ is at distance at least two from each end
of $D$, the copies $\mathrm{cut}-2,\dots,\mathrm{cut}+2$ all lie in $D$ and are
...
agrees with $\mathrm{TARGET}_n$ verbatim.

Now trace $\varphi$ in $S$ from any dart. Its counterpart trace in
$\mathrm{TARGET}_n$ closes within five steps inside a five-copy window, and $S$
is a translate of $\mathrm{TARGET}_n$ on that window, so each step of the trace
in $S$ reproduces the corresponding step and the walk closes at the same length.
Hence \emph{every face of $S$ is a translated face of $\mathrm{TARGET}_n$}.
...
Three facts in the proof are not bookkeeping and are discharged by computation
on the single certificate $\mathrm{TARGET}_n$: that every face has at most five
darts, that no strip edge joins copies more than two apart, and that the deep
block really is periodic, i.e.\ its rotations are translates of one another. All
three are finite checks on one graph, and none is a statement about $S$.
Everything else is the local argument above.
\end{remark}
...
the author-hosted corpus~\cite{APGcorpus}, together with the Section-8 closures
of \S\ref{sec:103}. Orders $37$ and $38$ are covered by the heuristic search of
\cite{AHSSV2015}; the disk surgery of this paper supplies independently checked
witnesses there, needed for \S\ref{sec:103} rather than to fill a gap in
Conjecture~10.2. Orders $46$ through $56$, $67$
...
window is a translated window'' is no longer literally true. The floor orders are therefore also carried by their own
verified certificates. The reach must be measured against the hypothesis
$|D| \ge 5$, not against the list of certificates. Five spliceable orders ---
$48, 51, 53, 54, 56$ --- have deep blocks of only $1,2,2,3,3$ copies and are not
eligible seeds; the eligible ones begin at $67, 68, 69$, so insertion reaches
nothing below $70$ and \textbf{all ten orders $57$--$66$ lie outside
Theorem~\ref{thm:pumping}}. None carries a stored certificate; they rest on the
verified floor-upwards splices. Two earlier versions of this remark named the
wrong set, the second because it was computed from the spliceable orders rather
than from the hypothesis --- being computed rather than asserted does not help
if the predicate is wrong. Stating and proving the deletion direction
separately, with its own cut rule, is left open.
\end{remark}
...
Remark~\ref{rem:deletion}, which puts orders $57$--$66$ on verified computation
rather than on the theorem; Theorem~8.1 is not needed. Orders $20$ through $45$ are inherited from that paper and are not
re-established here.
...
We state Theorem~\ref{thm:103} in this bounded form deliberately.
Conjecture~10.3 asks for every $n \ge 19$, and we do not prove that: the
remaining orders $57$--$66$, $75$--$87$, $93$--$108$ and $n \ge 111$ are carried
only by the spliced family, whose $3$-connectivity induction we have been unable
to establish. An earlier version of this paper stated the full conjecture as a
theorem; that was withdrawn on 2026-09-05.
...
\midrule
\multicolumn{2}{l}{\emph{withdrawn 2026-09-05:} $48$ and every $n \ge 50$, the spliced family} \\
\bottomrule
\end{tabular}
\end{center}
...
$18a+19b+20c+21d+3$, because $34$ and $35$ are not sums of $18,19,20,21$. They
are \emph{not} outside \cite{AHSSV2015} altogether --- its heuristic search
reaches every order from $20$ to $42$ --- but its \emph{construction} misses
them, so a witness built here is the first that is checkable from a stored
rotation system. They are obtained by
...
\emph{The infinite tail is not established.} An earlier version argued that the
spliced family is $3$-connected at every order it produces; that induction does
not go through and was withdrawn on 2026-09-05. Orders $57$--$66$, $75$--$87$,
$93$--$108$ and every $n \ge 111$ are open here. What follows records the
argument as it was made.

\emph{The claim was} that the tail is not an artefact of a search horizon. The spliced
...
The fourth problem of (\cite{AHSSV2015}, \S10, p.~362) asks for the asymptotic
density of $v_4/v_3$ on $[1,1.5]$, an interval Theorem~\ref{thm:32} forces. We
have nothing to offer on it, and add only a caution drawn from the data above.

All twenty-six certificates lie at the very bottom of the interval:
$v_4/v_3 \in [1.0000, 1.1250]$, drifting downwards with $n$ and attaining
exactly $1$ at orders $50$, $56$, $74$, $92$ and $110$. That is a selection
effect. The certificates come from one construction, and
Theorem~\ref{thm:pumping} extends it by inserting copies of a single period, so
every member inherits that period's degree profile; a construction emitting one
graph per order reports on the construction, not on the distribution. The
clustering at $1$ is what a periodic family looks like and what a sample would
not.

The methods here cannot reach the question. Everything proved above is an
existence statement --- one graph per order, or a class shown empty --- whereas
a density requires a measure over all graphs of order $n$, and nothing here
enumerates. The class is also rigid in a way that obstructs sampling: a sweep
over every two-edge switch of all twenty-six certificates finds none leaving a
valid $(3,4,5)$-alternating plane graph, so local moves cannot walk the space.
An exhaustive census at small orders, with \texttt{plantri}~\cite{plantri}
restricted to the profile, would give the first honest histogram; that is where
we would start.
...
alternation conditions are recomputed on every run by four separately written
...
The artifact runs $1308$ checks and needs only a Python interpreter; the
mathematics uses no third-party library. Two classes of check are load-bearing:
that each certificate is accepted by every verifier of its class --- the
order-$19$ witnesses are general alternating plane graphs and the three
$(3,4,5)$ procedures correctly reject them --- and that each source in
Theorem~\ref{thm:103} is necessary, in the sense that removing it reopens
orders. Most gates are paired with a control that must fail, so that a check
cannot pass by being vacuous. Not all: $17$ sites have no negative control, and in
September 2026 three coverage gates were found to be iterating an empty set.
Both are recorded in the artifact rather than corrected in silence; the two
alternation clauses are each controlled in the \emph{other} verifier, and every
certificate is run through both.
...
The artifact --- every certificate, all four decision procedures, the drawing
tool (a straight-line embedding after Tutte~\cite{Tutte1963}) and the export
tools, and the complete test suite --- is archived at
\cite{artifact}, under the MIT licence. The concept DOI
\href{https://doi.org/10.5281/zenodo.22269200}{10.5281/zenodo.22269200} always
...
Reproduce with \texttt{make deps \&\& make verify}. No third-party \emph{files}
are redistributed: no licence statement was found at the source of the published
corpus~\cite{APGcorpus}, so each of its graphs is re-expressed in the artifact's
own format with the SHA-256 of every original retained for comparison. The
graphs themselves are of course the corpus's; re-encoding changes the bytes, not
the provenance, and the artifact's licence is not asserted over them.
...
with witnesses for 10.3} --- certificates, verifiers and test suite, version 1.0.2, Zenodo, 2026.
...
\bibitem{APGcorpus}
I.~Alth\"ofer, \emph{Alternating plane graphs} --- status page and corpus of
published witnesses, \url{https://althofer.de/apg/apgs/}, accessed 1 September
2026.
