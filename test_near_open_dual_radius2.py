...
class NearOpenDualRadius2Tests(unittest.TestCase):
    def test_seed_k4_frontier_hashes_scores_and_exact_attempt_count(self) -> None:
        seed = json.loads(
        ...
        fixed, parents = near_open_dual_radius2.load_dual_k4_frontier(
            seed, k4_log
        )
        self.assertEqual(seed["source"]["sha256"], near_open_dual_radius2.EXPECTED_SOURCE_SHA256)
        self.assertEqual(seed["state_sha256"], near_open_dual_radius2.EXPECTED_SEED_STATE_SHA256)
        self.assertEqual(len(parents), 64)
        self.assertEqual(
            near_open_dual_radius2.parent_manifest_sha256(parents),
            near_open_dual_radius2.EXPECTED_PARENT_MANIFEST_SHA256,
        )
        ...
        self.assertEqual(
            ...
            near_open_dual_radius2.EXPECTED_ATTEMPTS,
        )
