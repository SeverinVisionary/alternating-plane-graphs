...
Conjecture~10.3, that a $3$-connected alternating plane graph exists for every
$n \ge 19$, is settled; we also exhibit one on $46$ vertices with a separating
pair. The fourth problem is untouched.
\end{abstract}
...
The artifact runs $1257$ checks. The four decision procedures use the Python
...
Reproduce with \texttt{make deps \&\& make verify}. No third-party graph data
is redistributed: no licence statement was found at the source of the published
corpus~\cite{APGcorpus}, so each of its graphs is re-expressed in the artifact's
own format with the SHA-256 of every original retained for comparison.
...
\bibitem{artifact}
H.~Yang, \emph{Settling Conjectures 10.1, 10.2 and 10.3 on alternating plane
graphs} --- certificates, verifiers and test suite, version 1.0.2, Zenodo, 2026.
